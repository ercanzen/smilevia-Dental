  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 px-3 pt-3 md:px-6 md:pt-5">
      <nav className="mx-auto flex h-16 max-w-[1440px] items-center justify-between rounded-full border border-white/50 bg-white/55 px-4 shadow-[0_10px_40px_rgba(10,17,48,.08)] backdrop-blur-2xl md:px-6">
        <Link to="/" className="flex items-center gap-3" onClick={() => setOpen(false)} aria-label="Smilevia Startseite">
        ...
        <div className="hidden items-center gap-5 lg:flex">
          {links.map((link) => <NavLink key={link.label} to={link.to} className={({isActive}) => `text-sm font-semibold transition hover:text-[#0a1130] ${isActive ? 'text-[#0a1130] underline decoration-[#4f6fd6] decoration-2 underline-offset-8' : 'text-[#454e70]'}`}>{link.label}</NavLink>)}
        </div>
        <div className="hidden items-center gap-3 md:flex">
          <LanguageSwitcher/>
          <Link to="/randevu-al" className="rounded-full bg-[#15265e]/90 px-5 py-3 text-sm font-bold text-white backdrop-blur-xl transition hover:-translate-y-0.5 hover:bg-[#1c3480]">Kostenlose Anfrage <span className="ml-1">↗</span></Link>
        </div>
      ...
      <div id="mobile-navigation" aria-hidden={!open} className={`absolute left-3 right-3 top-20 rounded-[28px] border border-white/15 bg-[#2c4956]/85 p-6 text-white shadow-2xl backdrop-blur-2xl transition-all md:hidden ${open ? 'visible translate-y-0 opacity-100' : 'invisible -translate-y-3 opacity-0'}`}>
        <div className="flex flex-col gap-1">
          {links.map((link) => <Link key={link.label} to={link.to} onClick={() => setOpen(false)} className="border-b border-white/10 py-4 text-2xl font-bold">{link.label}</Link>)}
          ...
          <Link to="/randevu-al" onClick={() => setOpen(false)} className="mt-3 rounded-full bg-[#4da8ff] px-5 py-4 text-center font-bold text-[#0a1130]">Kostenlose Anfrage</Link>
        </div>
      </div>
    </header>
  )
